...
//fun getRawHeaderText(file: File): String {
//    return try {
//        XWPFDocument(file.inputStream()).use { document ->
//            val policy = document.headerFooterPolicy
//            val sb = StringBuilder()
//            // Word docs can have 3 types of headers: Default, First Page, and Even Pages
//            val headers = listOfNotNull(
//                policy?.defaultHeader,
//                policy?.firstPageHeader,
//                policy?.evenPageHeader
//            )
...
//fun findValue(headerText: String, label: String): String {
//    if (headerText.isEmpty()) return "Header Empty"
//
//    // (?i) = Case Insensitive
//    // \\.? = Matches an optional period (useful for "No.")
//    // \\s* = Matches zero or more spaces
//    // (.*) = Captures the actual value
//    val regex = "(?i)$label\\.?\\s*:\\s*(.*)".toRegex()
//
//    val match = regex.find(headerText)
//
//    // Returns the captured group (index 1) or "Not Found" if no match
//    return match?.groupValues?.get(1)?.trim() ?: "Not Found"
//}
